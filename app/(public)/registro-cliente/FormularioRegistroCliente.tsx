...
export function FormularioRegistroCliente() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    const { data: auth, error: authErr } = await supabase.auth.signUp({
      ...
      options: { data: { role: 'cliente', nombre: form.nombre } },
    })
    ...
    const r = await fetch('/api/auth/registro-cliente', {
      method: 'POST',
  ...
  async function loginGoogle() {
    await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: {
        redirectTo: `${window.location.origin}/auth/callback?next=/mi-cuenta&role=cliente`,
      },
    })
  }
  ...
  if (mensaje) {
    return (
      <div className="text-center space-y-3 py-4">
        ...
        <Link href="/login" className="text-rojo font-medium text-sm hover:underline">Ir al login →</Link>
      </div>
    )
  }
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      <h3 className="font-display text-2xl text-azul font-bold">Crea tu cuenta gratis</h3>
      ...
      <Input label="Teléfono (opcional)" value={form.telefono} onChange={e => setForm({ ...form, telefono: e.target.value })} placeholder="+56 9 1234 5678" />

      <Button type="submit" loading={loading} className="w-full">Crear cuenta gratis</Button>

      <p className="text-xs text-gris-3 text-center">
        ¿Ya tienes cuenta? <Link href="/login" className="text-azul-mid hover:underline">Inicia sesión</Link>
      </p>
    </form>
  )
}
